...
func IfBlock(start, end int, queryText []string) {
	// for ever line in the if statement
	for i := start + 1; i <= end-1; i++ {
		queryText[i] = INDENTATION + queryText[i]

		// if there's a select find lines then skip those for indentation TODO write method to remove duplication
		if lower := strings.ToLower(queryText[i]); strings.Contains(lower, " select ") {

			// find which line the select statement ends on --> j
			for j := i + 1; j < len(queryText); j++ {
	...
	for i, line := range queryText {
		...
		if lower := strings.ToLower(line); strings.Contains(lower, "if ") {
			...
			for j := i; j < len(queryText); j++ {
				if txtLower := strings.ToLower(queryText[j]); !strings.Contains(txtLower, "end") {
					// correct indentation for the block
					fmt.Printf("txtLower: %v\n", txtLower)
					utils.IfBlock(i, j, queryText)
					break
				}
			}
		}
