...
class Parser {
    ...
    private Stmt ifStatement() {
        ...
        if (findMatch(ELSE)) // Else is bound to nearest if. GSQL is context-based
    ...
    private Expr equality() {
        ...
        while (findMatch(BANG_EQUAL, EQUAL_EQUAL)) {
            Token operator = previous();
    ...
    private Expr comparison() {
        Expr expr = term();
        while (findMatch(GREATER, GREATER_EQUAL, LESS, LESS_EQUAL)) {
            Token operator = previous();
            Expr right = term();
            expr = new Expr.Binary(expr, operator, right);
        }
    ...
    private Expr factor() {
        ...
        while (findMatch(SLASH, STAR, MODULO)) {
            Token operator = previous();
            Expr right = unary();
            expr = new Expr.Binary(expr, operator, right);
        }
    ...
    private Expr call() {
        ...
        while (true) {
            if (findMatch(LEFT_PAREN)) {
                expr = finishCall(expr);
            } else if (findMatch(DOT)) {
    ...
    private void synchronize() {
        ...
        while (!isAtEnd()) {
            if (previous().type == SEMICOLON) // doesn't work in a ; separated for loop
                return;
